class Archer {
    ...
    draw() {
        ...
        const armOffset = drawMan(xPos, yPos, step); // Draw the body, arms, and legs

        // === Knight Helmet ===
        ctx.fillStyle = "gray";
        ctx.beginPath();
        ctx.arc(xPos, yPos - 30, 10, 0, Math.PI * 2); // Helmet shape
        ctx.fill();
        ...
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.moveTo(xPos + 8*this.delta, yPos - 20+armOffset*this.delta);
        ctx.lineTo(xPos + 20*this.delta, yPos - 20+armOffset*this.delta);
        ctx.stroke();
        ...
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(xPos + 20*this.delta, yPos - 20+armOffset*this.delta);
        ctx.lineTo(xPos + 18*this.delta, yPos - 23+armOffset*this.delta);
        ctx.lineTo(xPos + 18*this.delta, yPos - 17+armOffset*this.delta);
        ctx.lineTo(xPos + 20*this.delta, yPos - 20+armOffset*this.delta);
        ctx.stroke();
